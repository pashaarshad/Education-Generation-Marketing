...
export default function Reports() {
  ...
  return (
    <section id="reports" className="py-24 relative overflow-hidden">
      ...
      {/* Background glow */}
      <div className="gradient-blob w-[500px] h-[500px] bg-brand-purple/10 top-[20%] left-[-10%] animate-pulse-slow" />
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center mb-16">
          ...
          <div className="lg:col-span-6 flex flex-col gap-4 border-l border-white/5 pl-0 lg:pl-8">
            <div className="flex items-start gap-3">
              <div className="w-5 h-5 rounded-full bg-brand-purple/10 flex items-center justify-center shrink-0 text-brand-violet mt-0.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {features.map((feat, idx) => (
            <motion.div
              ...
              key={idx}
              className="p-6 rounded-2xl bg-white/5 border border-white/5 flex flex-col gap-2 hover:border-brand-purple/20 transition-colors"
            >
              <h4 className="font-display font-bold text-base text-white">{feat.title}</h4>
              <p className="text-slate-400 text-xs md:text-sm leading-relaxed mt-1">{feat.desc}</p>
            </motion.div>
          ))}
        </div>
